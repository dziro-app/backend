...
pub async fn validate_third_token(third_api: String, code_info: Json<CodeInfo>, state: &State<AppState>, cookies: &CookieJar<'_>) ->  status::Custom<content::RawJson<String>>  {
  let mut profile_image = "https://tresubresdobles.com/wp-content/uploads/2021/04/skft-23aff38e10ee3c4e430a1f3450c4a01d.jpeg".to_string();

  let tmp_user: CreateUser; 
  ...
  match third_api.as_str() {
    ...
    "facebook" => {
      let user_data = facebook::get_user_info(code_info.code.clone()).await.unwrap();
